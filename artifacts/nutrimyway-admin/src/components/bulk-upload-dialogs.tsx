...
// ── Upload Centers Dialog ──────────────────────────────────────────────────────

export function UploadCentersDialog({
  onClose, onSuccess,
}: { onClose: () => void; onSuccess: (count: number) => void }) {
  const [rows, setRows] = useState<Record<string, unknown>[] | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-2xl border border-border shadow-xl w-full max-w-lg p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-foreground">Bulk Upload Centers</h3>
            <p className="text-xs text-muted-foreground">Create multiple centers in one file</p>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground text-sm">&#10005;</button>
        </div>
        <p className="text-xs text-muted-foreground">
          Required columns: <strong>id</strong> (lowercase letters, numbers, hyphens), <strong>name</strong>, <strong>password</strong> (min. 8 chars)
        </p>
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => {
            const headers = ["id", "name", "password"];
            const sample = [
              ["delhi-1", "Delhi - Janakpuri", "Delhi@Center1"],
              ["delhi-2", "Delhi - Dwarka", "Dwarka@Center2"],
              ["gurgaon-1", "Gurgaon - MG Road", "Ggn@Center1"],
            ];
            const ws = XLSX.utils.aoa_to_sheet([headers, ...sample]);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Centers");
            XLSX.writeFile(wb, "sample-centers-upload.xlsx");
          }} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
            <Download className="w-3.5 h-3.5" /> Download sample
          </button>
        </div>
        ...
        {rows && (
          <>
            ...
            <button onClick={() => void upload()} disabled={loading}
              className="flex items-center justify-center gap-2 w-full bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-40">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Upload Centers"}
            </button>
          </>
        )}
        {result && (
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-card rounded-2xl border border-border shadow-xl w-full max-w-lg p-6 space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          Upload an XLSX or CSV with columns: <strong>name</strong>, <strong>material_code</strong>, pack_size, pack_unit, description, flavour, serving_qty, kcal_per_serving, trial_eligible (yes/no)
        </p>
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => {
            const headers = ["name", "material_code", "pack_size", "pack_unit", "description", "flavour", "serving_qty", "kcal_per_serving", "trial_eligible"];
            const sample = [
              ["Almonds", "MAT-001", 100, "g", "Raw almonds", "nut", 10, 58, "no"],
              ["Whey Protein", "MAT-002", 500, "g", "Vanilla whey", "vanilla", 1, 120, "yes"],
            ];
            const ws = XLSX.utils.aoa_to_sheet([headers, ...sample]);
            const wb = XLSX.utils.book_new();
