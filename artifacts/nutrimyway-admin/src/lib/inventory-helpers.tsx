              ...
              <div className="mt-2 space-y-2">
                ...
                {adjustments.map(adj => (
                  <div key={adj.id} className="text-xs flex items-center justify-between p-2 bg-slate-50 border rounded-lg">
                    <span>{adj.note || "Adjustment"} <span className="text-muted-foreground">({fmtTime(adj.adjusted_at)})</span></span>
                    <span className="font-bold font-mono">{adj.qty_change > 0 ? '+' : ''}{adj.qty_change}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-2">
                {logs.map(log => (
                  <div key={log.id} className="flex items-center justify-between p-2.5 border rounded-lg hover:border-slate-300 transition-colors">
                    <div>
                      <div className="text-sm font-medium">Log #{log.id}</div>
                      <div className="text-xs text-muted-foreground">{fmtTime(log.recorded_at)}</div>
                    </div>
