...
# Replace checkin-menu
menu_api_old = '''// GET /api/members/:id/checkin-menu
...
menu_api_new = '''// GET /api/members/:id/checkin-menu
router.get("/members/:id/checkin-menu", async (req, res) => {
  const memberId = Number(req.params.id);
  ...
  const { rows: selections } = await pool.query(
    SELECT category_id, ingredient_id FROM visit_ingredient_selections WHERE checkin_id = ,
    [checkinId]
  );
...
content = content.replace(menu_api_old, menu_api_new)
